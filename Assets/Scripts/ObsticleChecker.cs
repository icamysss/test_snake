...
public class ObsticleChecker : MonoBehaviour
{
	...
	public CapsuleCollider colliderOC;
	...
	{
		Vector3 max = colliderOC.bounds.max;
		Vector3 min = colliderOC.bounds.min;
	...
	private void Update()
	{
		...
		Vector3 origin = footPos + normalized * (colliderOC.bounds.extents.x + cliffCheckAheadDist);
		origin.y = footPos.y + 0.1f;
		normalized = Quaternion.Euler(0f, 30f, 0f) * normalized;
		Vector3 origin2 = footPos + normalized * (colliderOC.bounds.extents.x + cliffCheckAheadDist);
		origin2.y = footPos.y + 0.1f;
		normalized = Quaternion.Euler(0f, -60f, 0f) * normalized;
		Vector3 origin3 = footPos + normalized * (colliderOC.bounds.extents.x + cliffCheckAheadDist);
	...
	private void OnDrawGizmos()
	{
		if ((bool)colliderOC)
		{
			...
			Gizmos.DrawCube(vector + (colliderOC.bounds.extents.x + cliffCheckAheadDist) * base.transform.forward, Vector3.one * 0.01f);
			Vector3 vector2 = Quaternion.Euler(0f, 30f, 0f) * base.transform.forward;
			Gizmos.DrawCube(vector + (colliderOC.bounds.extents.x + cliffCheckAheadDist) * vector2, Vector3.one * 0.01f);
